(function() {
    ...
    DIGESTERS = {
        ...
        extensionfor: function( tagname, value, block ) {
        ...
        pluginfor: function( tagname, value, block ) {
            var host = this.modules[ this.currentmodule ].classes,
                target = host[ this.resolveFor( value ) ],
                self = host[ block[ 'class' ] ];

            if ( self && target ) {
                self.plugin_for.push( target.name );
                target.plugins.push( self.name );
            }
        },
